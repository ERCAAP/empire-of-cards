...
namespace EmpireOfCards.Core
{
    public class TurnManager : MonoBehaviour
    {
        ...
        void FinishTurn()
        {
            ...
            // Check win/lose via WinLoseChecker
            var gm = GameManager.Instance;
            if (gm != null)
            {
                var res = gm.Resources;
                float rivalShare = Constants.MARKET_SHARE_MAX - res.GetMarketShare();

                var rivalAI = gm.RivalAI as Gameplay.RivalAI;
                if (rivalAI != null)
                    rivalShare = rivalAI.RivalShare;

                var (gameOver, won, reason) = WinLoseChecker.Check(
                    currentTurn,
                    res.GetMoney(),
                    res.GetRating(),
                    res.GetLegalRisk(),
