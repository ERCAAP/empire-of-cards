...
namespace EmpireOfCards.Save
{
    ...
    public class SaveManager : MonoBehaviour
    {
        ...
        public void UpdateMetaProgression(bool won, int finalScore, string grade)
        {
            SaveData save = HasSave() ? Load() : new SaveData();

            save.totalRuns++;
            if (won) save.totalWins++;

            if (finalScore > save.bestScore)
            {
                save.bestScore = finalScore;
                save.bestGrade = grade;
            }

            if (_gameManager != null)
            {
                int cash = _gameManager.Resources.GetMoney();
                save.totalCashEarned += cash;
                save.carryOverCash = cash * Constants.EXIT_CARRY_OVER_RATE;
            }

            save.activeRun = null;
            Save(save);
            Debug.Log($"[SaveManager] Meta updated. Runs: {save.totalRuns}, Wins: {save.totalWins}.");
        }
    }
}
